document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        //Agregado /api/ en la URL
        fetch('http://localhost/api/producer.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json'  // Agregado header Accept
            },
    ...
    function updateChart() {
        ...
        // Agregado /api/ en la URL
        fetch('http://localhost/api/get_bad_messages.php')
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
